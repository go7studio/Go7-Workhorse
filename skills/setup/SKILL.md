...
Desk skills include Grok, Codex, Claude, and **Workhorse** (`desk`, `setup`). Spark / Qwen login is the optional `dgx-spark` add-on (Import), not this skill. Leftover-meter wiring for a new vendor lives in the private skills hub (`vendor-meter`), not this public repo.
