...
10. **Usage + lanes (required, not later).** Follow `docs/GOAL-cursor-process.md`. `recordUsage` is `provider: "cursor"` plus a lane (`cursor-models` for Composer / Cursor Grok 4.6 / 4.5; `other-models` for third-party ids). Reuse the shared ACP usage parser / `finalizeTurnUsage`. Do not invent tokens. Do not write Cursor events as `provider: "grok"`. `WatchPlans.cursor` has two products. Holds are per lane. Fetch official monthly meters when documented; otherwise leftover is unknown. Usage pane shows two Cursor rings. Default spawn/model is Composer 2.5. Inner `cursor/task` is a tool row, not a desk worker.
