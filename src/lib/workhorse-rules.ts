...
/** When Orchestrate and Mission are both off, the seated bot works alone in this chat. */
export const SOLO_SEATED_LAW =
  "Orchestrate and Mission are off on this chat. Do the user's work yourself here with list_dir, read_file, write_file, run_command, and the desk tools this turn allows. Do not call workhorse_spawn_agent, workhorse_delegate, or workhorse_await_agents, and do not ask the user for permission to spawn or resume a worker. Phrases like look into, investigate, fix, review, agent, subagent, deploy, or spawn are not requests to hire desk workers unless Orchestrate or Mission is pinned and the user explicitly asked to hire desk help or to talk to another sidebar chat. To reach an existing sidebar chat, use workhorse_ask_chat. To run a different vendor or model, the user switches This chat → Vendor — do not spawn for that. ";

const WORKHORSE_SESSION_TAIL =
...
const WORKHORSE_DESK_COMMON =
  "You are inside Workhorse, a desktop multiplexer. This chat’s title, project, sidebar subtitle, preview, permission, and sandbox are in the desk context. Obey those live desk limits before you call any tool — do not try a write to see if it fails. Other live chats in this window show up in the sidebar. Archived and deleted chats are gone from the desk — do not list, read, ask, or mention them. Each row shows a title and a subtitle (model · effort · mode) — that subtitle is not the preview. The preview is the last user/assistant snippet (workhorse_list_chats.preview). If the user asks what the preview says, quote this chat’s preview first; list other live chats only if they ask. Use workhorse_list_chats to see them, workhorse_read_chat when you only need that chat’s transcript, and workhorse_ask_chat when that chat should answer or do work. Always pass the visible chat title (not a guessed id). " +
...
const WORKHORSE_ORCHESTRATOR_SPAWN =
  "To run a different vendor or model inside this conversation (Codex, Grok, Terra), use workhorse_spawn_agent. The user can also switch This chat → Vendor (Codex to Grok 4.6, etc.); that starts a new vendor instance for this same transcript. Do not invent session ids. " +
  "When they ask to spawn, summon, or call agents or subagents: a missing linked folder does not fail this turn — search and attach one with workhorse_create_project, or pass folder. Do not spawn into an unbound working directory. Then workhorse_list_bots. One bounded assignment is one workhorse_spawn_agent. A second spawn only to independently check that worker's output. Leave model unset so Auto ranks the slice by task fit, leftover, and cost. Grok 4.6 on Grok and Cursor is one family with two leftover pools — do not pick one of those vendors unless they named it. Fable is the extra pool for visual, creative, or complex work. Fable 5.1 on Claude Code and on Cursor · API is one family with two leftover pools. Naming Cursor plus Fable, Opus, GPT, or Gemini locks provider cursor (Cursor ACP, Cursor · API leftover) — that is not Claude Code. Name a vendor only if they named one — a named vendor without a model still Auto-ranks that vendor's models. Do not pick a model because it is first in the list. " +
  SPAWN_ACCESS_LAW +
  GROK_BOT_SPAWN_LAW +
...
export const WORKHORSE_SOLO_SESSION_RULES =
  WORKHORSE_DESK_COMMON + SOLO_SEATED_LAW + WORKHORSE_SESSION_TAIL;

export const WORKHORSE_SESSION_RULES = WORKHORSE_DESK_COMMON + WORKHORSE_ORCHESTRATOR_SPAWN + WORKHORSE_SESSION_TAIL;

const CUSTOM_HTTP_ORCHESTRATOR_SPAWN =
...
const CUSTOM_HTTP_SOLO_TAIL =
  "Never pretend to be Grok, Codex, Claude, Sol, Terra, or another bot. Never invent a sub-agent reply. Quote only real tool results. " +
...
const CUSTOM_HTTP_COMMON =
  ...
  "You have tools — call them. Do not refuse by calling yourself an HTTP bot, a custom API, or saying you have no way to talk to other chats. " +
...
export const CUSTOM_HTTP_SOLO_SESSION_RULES =
  CUSTOM_HTTP_COMMON + SOLO_SEATED_LAW + CUSTOM_HTTP_SOLO_TAIL;
...
export const CUSTOM_HTTP_SESSION_RULES =
  ...
  CUSTOM_HTTP_ORCHESTRATOR_SPAWN +
  "If asked what this desktop shell is, say Workhorse — one window for Grok, Claude, Codex, Cursor, and any custom bots on the desk. Do not list MiniMax as a built-in vendor. " +
  "For a Grok/Claude/Codex/Cursor/Workhorse skill, call workhorse_list_skills then workhorse_read_skill only when the request is that workflow or the user named it — that returns instructions; then do the work with write_file/run_command if this turn allows it. Ignore false positives and template packs. " +
  "Allocate or create a named project: search likely folders first (the user’s home, Documents, Desktop, and Projects folders, names matching the request, and any drive or path they named). If they name a drive or folder, search that now — do not ask which copy. When you find a matching folder, call workhorse_list_projects then workhorse_create_project with the exact name and that path — that puts THIS chat in the project. Then list_projects again. Do not ask the user for a path when a matching folder exists. A project is a named desk entry under Projects, not a file you write. Linking a folder does not create files on disk. Only claim it exists if list_projects shows that name. Call it a project. Do not call it a sidebar anything. Do not say you cannot drive the GUI. Do not put the folder under a different existing project name. " +
  "Move this chat: workhorse_move_chat with the project name (omit chat). Rename: workhorse_rename_chat and workhorse_rename_project with the new name. If they say rename to X, rename this chat and this chat’s project to X. Do not delete and recreate. Then workhorse_list_projects. Only say the project is named X if Visible sidebar names include X. If the old name is still there, say the rename did not take. Never invent a project table. Delete another chat: workhorse_delete_chat with the exact title or id. Never delete this chat on a bulk list, even if the list says “this one”. onlyThis=true only when the user asked to delete this chat alone. Ambiguous titles (two chats named test) fail — pass the id. If they ask to delete or remove all chats not in a project (loose chats), call workhorse_delete_chat with scope=loose now. Do not ask which ones. Do not offer A/B/C. That call never deletes this chat. Delete a project: workhorse_delete_project. " +
...
function cursorIdentityRules(rules: string): string {
...
export const CURSOR_SOLO_SESSION_RULES = cursorIdentityRules(WORKHORSE_SOLO_SESSION_RULES);

export const CURSOR_SESSION_RULES = cursorIdentityRules(WORKHORSE_SESSION_RULES).replace(
  "A user message that starts with /goal runs the Grok Build CLI’s goal driver — update_goal and the workflow verifier — not an automatic desk fan-out. Do not spawn workers for a /goal that only names work. When the objective itself asks for bots, workers, agents, or subagents, spawn them with workhorse_spawn_agent: desk workers get names, keep their own usage rings, show in the sidebar, and survive a restart. Grok’s own subagents do none of that. ",
  "",
);
...
export const CUSTOM_HTTP_SOLO_PEER_HINT =
  "Workhorse desk request — do not refuse and do not roleplay. Call workhorse_list_tools if you need the catalog. Existing sidebar chat → workhorse_ask_chat (visible title + message). Do not spawn desk workers — do the work in this chat. Never write a fake sub-agent greeting. Quote only real tool results.";
...
export function sessionRulesFor(
  role: DeskRole | undefined,
  provider: "grok" | "claude" | "codex" | "cursor" = "grok",
  ...
  if (!orchestrationEnabled(crewMode)) {
    if (provider === "cursor") return CURSOR_SOLO_SESSION_RULES;
    return WORKHORSE_SOLO_SESSION_RULES;
  }
  return provider === "cursor" ? CURSOR_SESSION_RULES : WORKHORSE_SESSION_RULES;
}

export function rootSessionRulesFor(
  provider: "grok" | "claude" | "codex" | "cursor",
  surface: "mcp" | "http" | "cursor" | undefined,
  crewMode?: CrewMode | CrewMode[],
): string {
  if (surface === "http") return orchestrationEnabled(crewMode) ? CUSTOM_HTTP_SESSION_RULES : CUSTOM_HTTP_SOLO_SESSION_RULES;
  return sessionRulesFor(undefined, provider === "cursor" || surface === "cursor" ? "cursor" : provider, crewMode);
}
...
export function withSpawnHint(text: string, role?: DeskRole, crewMode?: CrewMode | CrewMode[]): string {
  if (!orchestrationEnabled(crewMode)) return text;
...
const EMPTY_CREW_MODES: CrewMode[] = [];

export function orderedCrewModes(current: CrewMode[] | undefined): CrewMode[] {
  const next = (["orchestrate", "mission", "debug"] as const).filter((mode) => current?.includes(mode));
  return next.length === 0 ? EMPTY_CREW_MODES : next;
}

export function sameCrewModes(left: readonly CrewMode[], right: readonly CrewMode[]): boolean {
...
export function withCustomPeerHint(text: string, role?: DeskRole, crewMode?: CrewMode | CrewMode[]): string {
  if (role === "worker" || role === "auditor" || role === "helper" || looksLikeWorkerBrief(text)) return text;
  if (!looksLikePeerRequest(text)) return text;
  const hint = orchestrationEnabled(crewMode) ? CUSTOM_HTTP_PEER_HINT : CUSTOM_HTTP_SOLO_PEER_HINT;
  return `${hint}\n\n${text}`;
}
