...
2. Open Settings → Workshop (or **Manage** on the live rail once a pack is On, or **Workshop** in a chat header beside Review and Terminal). **Available** lists the first-party catalog (app-pin verified). Install binds archive digest and lands **Off** on this desk. **Local (Advanced)** still has From folder and optional repo URL (highest semver). Paste a public GitHub repo URL and press Add (the highest tagged release is downloaded, staged, validated, then installed), or press From folder to copy a pack folder. No `git` binary. A pack outside the vocabulary is refused with the exact field or "needs a newer Workhorse". Catalog summaries are display-only — never agent instructions or a playbook. Settings → Workshop is the install/grant home; the rail is live watch only. Skills is not the Workshop home. No dock item.
...
4. Live watch appears on the desk Workshop rail (collapsed strip → expand) only after Turn on. When no pack is On, the rail is hidden — Install and Turn on live in Settings → Workshop, the Manage sheet, and the chat Workshop basic. A Turn on that cannot go through names every missing piece and adds the Local Compute host in place. Optional Detach opens the breakout window. Manage sheet, Settings → Workshop, and the chat Workshop panel share one `WorkshopBlock`.
...
| Call | Does | Does not |
| --- | --- | --- |
| Renderer `updateWorkshop` (via rail Manage sheet, Settings → Workshop, or the chat Workshop panel) | Turns packs on or off and sets `hostId` and `sources` | Open or close the breakout by itself |
...
| `workshopInstallCatalog` (`workshop:install-catalog`) | Installs one catalog id with immutable source + digest bind | Highest-semver; silent sibling install; trust renderer-supplied digest |
| `workshopInstallRepo` (`workshop:install-repo`) | Advanced: highest tagged release of a public https repo | Run `git` or accept a private repo |
...
| `workshopUpdate` (`workshop:update`) | Re-installs the latest tag; before swap, turns every On pack whose sources changed off (including siblings in the same archive) and returns `reconfirm` / `reconfirmIds` | Widen a grant or keep an old grant against new URLs |
...
Live watch = desk Workshop rail (collapsed strip → expand; multi-pack stack; empty Manage chrome when none On). Manage sheet = install/grant/catalog (primary). Settings → Workshop = secondary. Chat Workshop basic = the same block beside Review and Terminal. Skills is not the Workshop home. Breakout = optional detach. See RAIL.md.
