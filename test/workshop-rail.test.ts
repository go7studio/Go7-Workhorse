...
import { clipLog, feedAge, feedReason, feedTone, flagWords, joinParts, primaryStatus, probeRows } from "../src/ui/workshop-live";
...
test("a source that is not painting says why, in words, instead of only a dash", () => {
  // Nothing wrong, nothing to say.
  assert.equal(feedReason(undefined), undefined);
  assert.equal(feedReason({ present: true, asOf: "2026-09-09T00:00:00.000Z" }), undefined);
  assert.equal(feedReason({ present: false, reason: "off" }), undefined);

  // The refusal the operator meets first: the gateway is up and the credential is not accepted.
  assert.equal(feedReason({ present: false, reason: "unauthorized" }), "host refused the credential");
  assert.equal(feedReason({ present: false, reason: "token" }), "credential file unreadable");
  assert.equal(feedReason({ present: false, reason: "no-host" }), "no Local Compute host");
  assert.equal(feedReason({ present: false, reason: "unreachable" }), "host unreachable");
  assert.equal(feedReason({ present: false, reason: "missing" }), "gateway has no such path");
  assert.equal(feedReason({ present: false, reason: "path" }), "gateway has no such path");
  assert.equal(feedReason({ present: false, reason: "schema" }), "reply did not match the pack's schema");
  assert.equal(feedReason({ present: false, reason: "too-large" }), "reply over the pack's byte cap");

  // Any other status the host reports still reads as a sentence, never as a bare code.
  assert.equal(feedReason({ present: false, reason: "http-503" }), "host answered 503");
  // An unknown word is passed through rather than swallowed.
  assert.equal(feedReason({ present: false, reason: "weather" }), "weather");
});
