...
test("Grok 4.6 on Grok Build and Cursor is one leftover family", () => {
  ...
  assert.equal(routingModelFamily({ provider: "claude", model: "claude-fable-5-1" }), "fable-5-1");
  assert.equal(routingModelFamily({ provider: "cursor", model: "claude-fable-5-1" }), "fable-5-1");
  assert.equal(spawnModelFamilyKey("Fable 5.1"), "fable-5-1");
  assert.equal(spawnModelFamilyKey("claude-fable-5-1"), "fable-5-1");
  assert.equal(shouldAutoRouteSpawn({ routingEnabled: true, model: "Fable 5.1" }), true);
  assert.equal(shouldAutoRouteSpawn({ routingEnabled: true, provider: "cursor", model: "Fable 5.1" }), false);
  const fableFamily = constrainRouteCandidatesForSpawn(
    [
      {
        provider: "claude",
        ...
        label: "Fable 5.1",
        connected: true,
        profile: routingProfileForModel("claude", "claude-fable-5-1"),
      },
      {
        provider: "cursor",
        model: "claude-fable-5-1",
        ...
        connected: true,
        profile: routingProfileForModel("cursor", "claude-fable-5-1"),
      },
      grok46(20),
    ],
    { model: "Fable 5.1" },
  );
  assert.deepEqual(fableFamily.map((row) => row.provider).sort(), ["claude", "cursor"]);
