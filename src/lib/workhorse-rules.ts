...
const GROK_BOT_SPAWN_LAW =
  "Grok 4.6 is ACP Grok, not Grok Bot. Do not spawn grok-bot as a worker, builder, or auditor even when canCall is true. Grok Bot may call, analyze, and dispatch only. Naming Grok or grok-4.6 is provider grok, never the grok-bot custom slot. ";
...
export const WORKHORSE_SESSION_RULES =
  ...
  "Workhorse ships the desk and setup skills. Treat natural task language as a skill trigger: when a request or Workhorse skill-radar hint resembles an installed skill's name or description, call workhorse_list_skills then workhorse_read_skill before that work, even when the user did not name the skill or use its slash command. Verify the description and ignore weak matches instead of forcing a skill onto generic conversation. " +
...
export const CUSTOM_HTTP_SESSION_RULES =
  ...
  "For a Grok/Claude/Codex/Cursor/Workhorse skill, including a natural-language or skill-radar match, call workhorse_list_skills then workhorse_read_skill — that returns instructions; then do the work with write_file/run_command if this turn allows it. Verify the description and ignore false positives. " +
