...
2. One bounded assignment is one `workhorse_spawn_agent`. A second spawn only to independently check that worker’s output. Leave `model` unset so Auto ranks the slice. Name a vendor only if they named one — a named vendor without a model still Auto-ranks that vendor’s models. Do not pick a model because it is first in the list. Spawn only a `canCall` row. The prompt is the **slice**.
