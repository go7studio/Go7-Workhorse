...
| **Opening text** | A chat opens with a core: desk tools, permissions, files, the vendor boundary. The spawn law is a separate block, and it reaches a chat only on a spawn-shaped turn or an Orchestrate or Mission pin. A custom HTTP bot pays its core again on every request, so its core is marked cacheable in the shape Anthropic documents. Every rule sentence in the old bible is listed in `test/fixtures/opening-text-rules.json` and proved still to reach the same chat. A shortened sentence also records what it keeps: the condition, number, exception or named object it holds on to. Without that, a rewording proves only that some string exists, which is how "call workhorse_delete_chat with scope=loose when they ask" passed as a standing order to wipe every loose chat. The character ceiling cannot be met by dropping a rule, and no longer by compressing one until it says something else. | `test/opening-text-is-lean.test.ts` — "no rule sentence was lost when the opening text was split", "every rewording says what it keeps", "the loose-chat delete keeps its condition", "the five named laws survive word for word", "the spawn law reaches a chat that can spawn, and no other", "a chat that never got the spawn law may not spawn", "the opening text stays under its ceiling for every role" |
