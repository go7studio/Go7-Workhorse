...
- **A chat starts, then you pick the model.** The composer menu holds vendor, model (for example Grok 4.7 or GPT-5.6-Sol), and brain effort. A vendor client such as the Grok Build CLI or Codex CLI is not a model. Do not put a brain picker in front of New chat.
...
- **Settings holds Profile, connected LLMs, skills, workshop, bot knowledge, routing, learning, usage, watch.** Usage is not a top-level sidebar item. Adapters call `recordUsage`. Do not invent tokens for preview chats.
...
- Grok (through the Grok Build CLI), Codex, Claude, and Cursor are live ACP adapters (Electron main only). Client/runtime identity and model identity stay separate. Custom is live HTTP from a pasted Anthropic/OpenAI-compatible URL and key. Never invent a login. Cursor usage is two pools (Composer/Cursor Grok vs Other Models). Do not fold Cursor into Grok. Custom HTTP includes a Grok Bot preset on 127.0.0.1:8787, model `grok-bot` — not ACP Grok (`grok-4.7`). Auto must not allocate grok-bot as an orchestration or builder worker; it may call, analyze, and dispatch. Naming Grok or `grok-4.7` executes the Grok ACP vendor. Its weekly leftover reads `grok-bot-leftover.json` into that bot’s custom plan slot only. Workhorse never writes that file.
- **Workhorse Link** is how an outside runtime talks to this desk: capabilities, capacity, chats, read, ask, delegate. Connect Grok is `grok mcp add` (TUI). Connect Cursor writes `~/.cursor/mcp.json` (no `mcp add`). Connect Grok Bot is the same launch plus a one-shot paste; Grok Bot must persist it off the remote box. Workhorse owns that shim on Mac and Windows. Do not rebind the desk bridge. Connecting Link is not adding that app as a vendor.
