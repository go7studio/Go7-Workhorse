...
const DESK_OBJECT_LAW =
  ...
  "Call workhorse_list_skills then workhorse_read_skill when a request names an installed workflow or skill radar lists a genuine match. Ignore weak keyword overlap. Ignore false positives and template packs. Do not load a skill because a product or format name appeared in passing. ";
 ...
 * tool with none of the law. This is the line that holds whatever the
 * phrasing: no spawn until the law arrives, and ask for it in words.
 */
export const SPAWN_GATE_LAW =
  "Do not call workhorse_spawn_agent on a turn that did not bring you the desk spawn law: say the desk can put workers on this, ask the user to say so, and wait. ";
