...
import { hydrateChatImages, hydrateChatImagesForHistory } from "./attachment-store";
...
export class CustomSessionHost {
  ...
  private async promptUnlocked(input: CustomPromptInput, emit: GrokEventSink): Promise<GrokPromptResult> {
    ...
    const history = (input.history ?? [])
      ...
      .map((item) => (item.images?.length ? { ...item, images: hydrateChatImagesForHistory(item.images) } : item));
