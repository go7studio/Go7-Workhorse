...
2. One bounded assignment is one `workhorse_spawn_agent`. A second spawn only to independently check that worker’s output. Leave `model` unset so Auto ranks the slice by task fit, leftover, and cost. Fable is the extra pool for visual, creative, or complex work. Name a vendor only if they named one — a named vendor without a model still Auto-ranks that vendor’s models. Do not pick a model because it is first in the list. Spawn only a `canCall` row. The prompt is the **slice**. Grok 4.6 is ACP Grok, not Grok Bot. Do not spawn `grok-bot` as a worker, builder, or auditor even when `canCall` is true. Grok Bot may call, analyze, and dispatch only. Naming Grok or `grok-4.6` is provider grok, never the grok-bot custom slot.
