...
export function createWorkshopHost(options: WorkshopHostOptions) {
  ...
  /** Host id -> the address and credential file its groups last read through. */
  const hostPrints = new Map<string, string>();
  ...
  function refresh(): void {
    ...
    // A host that was re-pointed or given a different credential file deserves an immediate retry.
    // The share key carries only the host id, so without this a group that backed off after repeated
    // refusals keeps that backoff long after the operator fixed the host. Prints are collected for
    // every host first: two groups can share one host, and the second must see the same old print.
    const prints = new Map<string, string>();
    for (const group of groups.values()) {
      if (prints.has(group.hostId)) continue;
      const host = hostFor(group.hostId);
      // Neither field can hold a newline, so it separates them unambiguously.
      prints.set(group.hostId, host ? `${host.baseUrl}\n${host.tokenFile}` : "");
    }
    const repointed = new Set<string>();
    for (const [hostId, print] of prints) {
      const known = hostPrints.get(hostId);
      if (known !== undefined && known !== print) repointed.add(hostId);
    }
    // Replace wholesale so a host whose last group went away leaves nothing behind.
    hostPrints.clear();
    for (const [hostId, print] of prints) hostPrints.set(hostId, print);
    for (const group of groups.values()) {
      if (!repointed.has(group.hostId) || newGroups.includes(group)) continue;
      group.failures = 0;
      if (group.timer) clearTimeout(group.timer);
      group.timer = null;
      void runGroup(group);
