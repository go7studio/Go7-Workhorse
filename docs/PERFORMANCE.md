...
| **Opening text** | A chat opens with a core: desk tools, permissions, files, the vendor boundary. The spawn law is a separate block, and it reaches a chat only on a spawn-shaped turn or an Orchestrate or Mission pin. A custom HTTP bot pays its core again on every request, so its core is marked cacheable in the shape Anthropic documents. Every rule sentence in the old bible is listed in `test/fixtures/opening-text-rules.json` and proved still to reach the same chat, so the character ceiling cannot be met by dropping a rule. | `test/opening-text-is-lean.test.ts` — "no rule sentence was lost when the opening text was split", "the five named laws survive word for word", "the spawn law reaches a chat that can spawn, and no other", "the opening text stays under its ceiling for every role" |
