...
/** Plain words for the host's reason vocabulary, so a refused read says why. */
const FEED_REASON_COPY: Record<string, string> = {
  unauthorized: "host refused the credential",
  token: "credential file unreadable",
  missing: "gateway has no such path",
  path: "gateway has no such path",
  unreachable: "host unreachable",
  "no-host": "no Local Compute host",
  malformed: "reply was not readable",
  schema: "reply did not match the pack's schema",
  "too-large": "reply over the pack's byte cap",
  stale: "feed is older than the pack allows",
};

/**
 * One human sentence fragment for a source that is not painting, or undefined when it is fine.
 * The chip already shows the raw word; this is what a surface prints beside it so a refusal reads
 * as a cause rather than as a dash.
 */
export function feedReason(status: SourceStatus | undefined): string | undefined {
  const reason = status?.reason;
  if (!reason || reason === "off") return undefined;
  const known = FEED_REASON_COPY[reason];
  if (known) return known;
  const http = /^http-(\d{3})$/.exec(reason);
  return http ? `host answered ${http[1]}` : reason;
}
