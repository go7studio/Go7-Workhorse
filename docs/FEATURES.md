...
- An input that fails gets a fixed number of attempts, counted against the input rather than the run, on every path including the resume after a restart. A rate limit or a busy endpoint says nothing about the input and does not spend that budget. Once the attempts are spent the desk stops asking, steps past the evidence it cannot compile so later events still get their turn, waits longer between tries while failures repeat, and records one line per failure instead of a stack.
